...
// Handle "change" type
if ($type === 'change') {
    $stmt = $conn->prepare("SELECT url FROM po_documentsdocuments WHERE ord_id = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        $deleteStmt = $conn->prepare("DELETE FROM po_documentsdocuments WHERE ord_id = ?");
...
// Insert new records
$successCount = 0;
$stmt = $conn->prepare("INSERT INTO po_documentsdocuments (ord_id, url, filename) VALUES (?, ?, ?)");
