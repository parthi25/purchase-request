...
// --- Collect input data aligned to payload ---
$po_id = validateInteger($_POST['ids'] ?? null);
$status = 7;
$po_num = validateInteger($_POST['PO'] ?? null);
$po_qty = validateInteger($_POST['qty'] ?? null);
$po_lines = validateInteger($_POST['Lines'] ?? null);

$status_date = sanitizeString($_POST['status_date'] ?? date('Y-m-d H:i:s'));
$po_date = sanitizeString($_POST['PODate'] ?? '');
$buyer = sanitizeString($_POST['buyerInput'] ?? '');
...
if (!$po_num)
    $errors['PO'] = 'Valid PO number is required';
if (!$po_qty)
    $errors['qty'] = 'Valid PO quantity is required';
if (!$po_lines)
    $errors['Lines'] = 'Valid PO lines are required';
